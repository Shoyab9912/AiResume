...
<div class="card">
  ...
  ${Object.entries(result.scoreBreakdown)
    ...
    <div class="bar-row">
      <div class="bar-hd">
        <span style="text-transform:capitalize">${escapeHtml(key)}</span>
        <strong>${val.score}/100</strong>
      </div>
      <div class="bar-track">
        <div class="bar-fill" style="width:${val.score}%"></div>
      </div>
      <div class="bar-fb">${escapeHtml(val.feedback)}</div>
    </div>`,
    )
    .join("")}
</div>
...
<div class="card">
  ...
  ${result.strengths
    .map((s) => `<div class="strength"><span>✓</span>${escapeHtml(s)}</div>`)
    .join("")}
</div>
...
<div class="card">
  ...
  ${result.suggestions
    ...
    <div class="sug ${s.priority}">
      <div class="sug-hd">
        <span class="sug-cat">${escapeHtml(s.category)}</span>
        <span class="sug-prio">${prioEmoji[s.priority]} ${escapeHtml(s.priority)}</span>
      </div>
      <div class="sug-issue">${escapeHtml(s.issue)}</div>
      <div class="sug-rec">→ ${escapeHtml(s.recommendation)}</div>
    </div>`,
    )
    .join("")}
</div>
